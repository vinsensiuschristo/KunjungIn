...
class MainViewModel(private val repository: DestinationRepository) : ViewModel() {
    ...
    val topRatedPlacesResponse: LiveData<TopRatedPlacesResponse> = repository.topRatedPlacesResponse
    ...
    fun getTopRatedPlaces(placeName: String, types: String, cityId: Int, token: String) {
        viewModelScope.launch {
            repository.getTopRatedPlace(placeName, types, cityId, token)
        }
    }
